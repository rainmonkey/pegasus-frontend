...
export class DownloadPDFService {
  ...
  downloadPDF(learnerName: IInvoiceLearnerName, invoice: IInvoice ,branch:IBranch) {
    ...
    // detail
    doc.setFontSize(8);
    doc.text(`ADDRESS1：${splitAddress(branch.Address).firstPart}`, 14, 60);
    doc.text(`INVOICE DATE ${invoice.BeginDate.slice(0, 10)}`, 140, 60)
    doc.text(`ADDRESS: ${splitAddress(branch.Address).secondPart}`, 14, 70);
    doc.text(`INVOICE NUMBER ${invoice.BeginDate.slice(0, 10)}`, 140, 70)
    doc.text(`PHONE: ${branch.Phone}`, 14, 80);
    doc.text(`E-MAIL: ${branch.Email}`, 14, 90);
